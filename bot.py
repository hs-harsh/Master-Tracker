#!/usr/bin/env python3
"""
Claude Telegram Bot — lives at root of Portfolio-Tracker repo.
/app = full repo. No cloning needed.
"""
...
GITHUB_REPO       = "https://github.com/hs-harsh/Portfolio-Tracker.git"
PROJECT_FOLDER    = "/app"  # Dockerfile puts full repo here — no cloning needed
...
def setup_repo():
    """Clone Portfolio-Tracker into /repo so the bot has full access."""
    ...
    if repo_path.exists():
        ...
        subprocess.run(["git", "pull"], cwd=PROJECT_FOLDER, capture_output=True)
    else:
        print(f"📥 Cloning Portfolio-Tracker → {PROJECT_FOLDER}")
        result = subprocess.run(["git", "clone", clone_url, PROJECT_FOLDER], capture_output=True, text=True)
...
def main():
    setup_git()  # Configure git identity + authenticated remote
